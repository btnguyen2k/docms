<template>
  ...
  <div v-else-if="!topic.id" class="alert alert-info m-4" role="alert">{{ $t('error_topic_not_found', {topic: $route.params.tid}) }}</div>
            ...
            <div class="doc-content col-md-9 col-12 order-1">
              <div class="content-inner">
                <section class="doc-section">
                  ...
                  <div v-for="doc in documentList" v-bind:key="doc.id" class="section-block">
                    <h3 class="block-title"><fa-icon v-if="doc.icon!=''" :icon="doc.icon" class="pe-1"/>{{ _localedText(doc.title) }}</h3>
                    <p>{{ _localedText(doc.summary) }}</p>
                  </div>
                </section>
              </div>
            </div>
